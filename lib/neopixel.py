#https://github.com/blaz-r/pi_pico_neopixel/blob/main/examples/colorwave.py
import array, time
...
class Neopixel:
    ...
    def fade_pixel(self, pixel_num, rgb1, rgb2, cstep, steps):
        sh_R, sh_G, sh_B, sh_W = self.shift
        if (cstep >= 0 and cstep <= steps ):
            red = round(rgb1[0] + (rgb2[0] - rgb1[0]) * cstep / steps)
            green = round(rgb1[1] + (rgb2[1] - rgb1[1]) * cstep / steps)
            blue = round(rgb1[2] + (rgb2[2] - rgb1[2]) * cstep / steps)
            white = 0
    ...
    def fade_pixel_line(self, pixel1, pixel2, rgb1, rgb2, cstep, steps):
        for i in range(pixel1, pixel2):
            self.fade_pixel(i, rgb1, rgb2, cstep, steps)